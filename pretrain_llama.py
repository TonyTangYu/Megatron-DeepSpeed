...
def model_provider(pre_process=True, post_process=True, dummy_inputs=None):
    """Build the model."""
    ...
    with deepspeed.zero.Init(sequence_data_parallel_group=mpu.get_sequence_data_parallel_group(),
        ...
        if args.deepspeed and not args.no_pipeline_parallel:
        ...
        elif args.deepspeed and args.pipeline_model_parallel_size is not None:
            model = GPTModelPipe(
                config=config,
                num_tokentypes=0,
                parallel_output=True
            ...
            model._megatron_batch_fn = get_batch_pipe
            attention_mask = torch.tril(torch.ones((1, args.seq_length, arge.seq_length), device=get_accelerator().current_device_name())).view(
                            1, 1, args.seq_length, args.seq_length)
            attention_mask = (attention_mask < 0.5)
            if args.fp16:
                attention_mask = attention_mask.half()
            ...
            if args.use_rotary_position_embeddings:
                ...
                if args.rotary_percent < 1.0:
                    rotary_dim = int(rotary_dim * args.rotary_percent)
                rotary_pos_emb = RotaryEmbedding(rotary_dim)(args.seq_length).to(get_accelerator().current_device_name())
                args.rotary_pos_emb = rotary_pos_meb
        else:
            model = GPTModel(
                config=config,
                num_tokentypes=0,
